...
void JoueurNoir::initialiserPieces()
{
	for (int i = 0; i < 16 ; i++)
		{
			if (((i%8)+1==3 || (i%8)+1==6) && i>=8)
				p[i] = new Fou((i%8)+1, 8,false);
			else if (((i%8)+1==2 || (i%8)+1==7) && i>=8)
				p[i] = new Cavalier((i%8)+1, 8,false);
			else if (((i%8)+1==1 || (i%8)+1==8) && i>=8)
				p[i] = new Tour((i%8)+1, 8,false);
			else if ((i%8)+1==4 &&  i>=8)
				p[i] = new Dame ((i%8)+1, 8,false);
			else if ((i%8)+1==5 &&  i>=8)
				p[i] = new Roi ((i%8)+1, 8,false);
			else if (i<8)
				p[i] = new Pion ((i%8)+1, 7,false);
		}
}
...
void JoueurNoir::choisirPiece(Echiquier &e)
{
	int x,y;
	string position="";
	cout << endl;
	e.affiche();
	cout << "\n\t\t\t \033[1;30mAu NOIR de joueur.\033[0m\n" << endl;
	cout << "Choisir la position du piece(ex: e7): ";
	cin >> position;
	...
	if ( p == 0 )
	{
		...
		{
			system("clear");
			cout << "\033[1;31m\t\t Erreur! Le piece est de JoueurBlanc!\033[0m";  
			choisirPiece(e);
		}
	return;
}

void JoueurNoir::choisirPosition(Echiquier &e, Piece *p)
{
	string mouvement="";
	int x,y;

	cout << "Choisir la position pour mouvement(ex: e5): ";
	cin >> mouvement;
	if (mouvement[2]!='\0' && mouvement[2]!=' ')
	{
		system("clear"); 
	...
	switch (mouvement[0])
	{
		...
		default:	system("clear");
					cout << endl;
					e.affiche(); 
					cout << "\033[1;31mLa position pour deplacer est erreur!\033[0m\n\n";
					choisirPosition(e,p); 
					break;
	}
	...
	if (p->mouvementValide(e,x,y)) // si la position x,y est valide, afficher le succes pour deplacer
	{
		system("clear");
...
bool JoueurNoir::EstFinJeu(Echiquier &e){
	int x_roi,y_roi;
	for (int i = 0; i < 16 ; i++)
	{
		...
		for (int y=1;y<=8;y++)
			if (e.getPiece(x,y)!=NULL && e.getPiece(x,y)->isWhite())
			{
				if (e.getPiece(x,y)->mouvementValide(e,x_roi,y_roi))
					{
						cout << "\033[1;31mSystem: JoueurNoir est pres de perdre. Faire plus attention!!!\033[0m"<< endl;
						if (Total_MouvementValide(x_roi,y_roi,e)==0) 
							{
								if (Sacrifier(x,y,x_roi,y_roi,e))
									return false;
								else return true;
							}
						else return false;
...
bool JoueurNoir::Sacrifier(int x,int y,int x_roi,int y_roi,Echiquier &e)
{
	for (int m=1;m<=8;m++)
		for (int n=1;n<=8;n++)
			if (e.getPiece(m,n)!=NULL && e.getPiece(m,n)->isBlack())
			{
...
int JoueurNoir::Total_MouvementValide(int x,int y, Echiquier &e){
	int nb=0;
	Piece *p=e.getPiece(x,y);
	for (int i=1; i<=8; i++)
		for (int j=1; j<=8; j++)
...
bool JoueurNoir::EstEncoreEchecEtMat(Echiquier &e){
	int x_roi,y_roi;
	for (int i = 0; i < 16 ; i++)
	{
		if (p[i]!=NULL && typeid(*p[i]) == typeid(Roi))
